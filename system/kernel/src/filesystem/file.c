...
static struct FAT12File files[256] = {0};
...
int32_t NAMESPACE(fopen)(const char *fileName, uint32_t flags) {
    ...
    struct FAT12 fat12;
    fat12Init(&fat12, kernel_floppy_loadSector);
    struct FAT12File file = fat12GetFileByName(&fat12, fat12FileName);
    if (!fat12FileExists(&file)) {
        return -1;
    }
    int32_t i = 0;
    for (; i < 256; ++i) {
        if (!fat12FileExists(files + i)) {
            files[i] = file;
            break;
        }
    }
